...
describe('ERC1155 computeCreate2Address', () => {
  testAll((chainId, token) => {
    ...
    test(`${chainIdToString(chainId)} - ${symbol} ${address}`, () => {
      const computed = bitagent.network(chainId).nft(symbol).getTokenAddress();
...
describe('ERC1155 token exists', () => {
  testAll((chainId, token) => {
    ...
    test(`${chainIdToString(chainId)} - ${symbol} ${address}`, async () => {
      const exists = await bitagent.network(chainId).nft(symbol).exists();
...
describe('ERC1155 token does NOT exist', () => {
  testAll((chainId, token) => {
    ...
    test(`${chainIdToString(chainId)} - ${fakeSymbol}`, async () => {
      const exists = await bitagent.network(chainId).nft(fakeSymbol).exists();
...
describe('ERC1155 decimals should equal 0', () => {
  testAll((chainId, token) => {
    ...
    test(`Chain ${chainIdToString(chainId)} - ${symbol} ${address}`, async () => {
      const decimals = await bitagent.network(chainId).nft(symbol).getDecimals();
...
describe('ERC1155 read name, symbol, and totalSupply', () => {
  testAll((chainId, token) => {
    const { symbol, address } = token;
    test(`Chain ${chainIdToString(chainId)} - ${symbol} ${address}`, async () => {
      const name = await bitagent.network(chainId).nft(symbol).getName();
      const tokenSymbol = await bitagent.network(chainId).nft(symbol).getSymbol();
      const totalSupply = await bitagent.network(chainId).nft(symbol).getTotalSupply();
...
describe('ERC1155 balance of fake address should be 0n', () => {
  testAll((chainId, token) => {
    ...
    test(`Chain ${chainIdToString(chainId)} - ${symbol} FakeAddress: ${fakeAddress}`, async () => {
      const balance = await bitagent.network(chainId).nft(symbol).getBalanceOf(fakeAddress);
