...
import { bitagent, MainnetChain, chainIdToString } from '../../../src';
...
describe('ERC20 computeCreate2Address', () => {
  testAll((chainId, token) => {
    ...
    test(`${chainIdToString(chainId)} - ${symbol} ${address}`, () => {
      const computed = bitagent.network(chainId).token(symbol).getTokenAddress();
...
describe('ERC20 token exists', () => {
  testAll((chainId, token) => {
    ...
    test(`${chainIdToString(chainId)} - ${symbol} ${address}`, async () => {
      const exists = await bitagent.network(chainId).token(symbol).exists();
...
describe('ERC20 token does NOT exist', () => {
  testAll((chainId, token) => {
    ...
    test(`${chainIdToString(chainId)} - ${fakeSymbol}`, async () => {
      const exists = await bitagent.network(chainId).token(fakeSymbol).exists();
...
describe('ERC20 decimals should equal 18', () => {
  testAll((chainId, token) => {
    ...
    test(`Chain ${chainIdToString(chainId)} - ${symbol} ${address}`, async () => {
      const decimals = await bitagent.network(chainId).token(symbol).getDecimals();
...
describe('ERC20 read name, symbol, and totalSupply', () => {
  testAll((chainId, token) => {
    const { symbol, address } = token;
    test(`Chain ${chainIdToString(chainId)} - ${symbol} ${address}`, async () => {
      const name = await bitagent.network(chainId).token(symbol).getName();
      const tokenSymbol = await bitagent.network(chainId).token(symbol).getSymbol();
      const totalSupply = await bitagent.network(chainId).token(symbol).getTotalSupply();
...
describe('ERC20 balance of fake address should be 0n', () => {
  testAll((chainId, token) => {
    ...
    test(`Chain ${chainIdToString(chainId)} - ${symbol} FakeAddress: ${fakeAddress}`, async () => {
      const balance = await bitagent.network(chainId).token(symbol).getBalanceOf(fakeAddress);
