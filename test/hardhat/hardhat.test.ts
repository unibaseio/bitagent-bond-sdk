...
// import { bitagent as sdk } from '../../src';
...
//   const bitagent = sdk.withPublicClient(publicClient);
...
//       const balance = await bitagent.withWalletClient(bob).getNativeBalance();
...
//       const exists = await bitagent.token(wonderlandTokenAddress).exists();
//       expect(exists).toEqual(true);
//     });

//     test(`Check if ${WONDERLAND_TOKEN_CURVE.symbol} token has correct data`, async () => {
//       const { reserveToken, burnRoyalty, mintRoyalty, creator } = await bitagent
...
//       const balance = await bitagent.token(wonderlandTokenAddress).getBalanceOf(alice.account.address);
...
//       const allowance = await bitagent.withWalletClient(alice).token(ReserveToken.address).getAllowance({
...
//       const price = await bitagent.token(wonderlandTokenAddress).getPriceForNextMint();
...
//       const balance = await bitagent.token(wonderlandTokenAddress).getBalanceOf(bob.account.address);
//       expect(balance).toEqual(0n);
//     });

//     test(`She gives one to bob`, async () => {
//       await bitagent.token(wonderlandTokenAddress).transfer({
...
//       await bitagent.withWalletClient(alice).token(wonderlandTokenAddress).approve({
//         tradeType: 'sell',
//         onSignatureRequest: assertTxSignature,
//         onSuccess: assertTxSuccess,
//       });
...
//       const exists = await bitagent.token(computeCreate2Address(hardhat.id, 'ERC20', BOB_TOKEN_STEP.symbol)).exists();
//       expect(exists).toEqual(true);
//     });
